...
@section('title', ucfirst($portal) . ' Login')
...
@section('content')
    ...
    <div class="flex items-center gap-1 bg-line/50 rounded-full p-1 mb-8 w-fit">
        <a href="{{ route('login.student') }}"
           class="px-4 py-2 rounded-full text-sm font-semibold transition-colors {{ $activePortal === 'student' ? 'bg-white text-ink shadow-sm' : 'text-ink/50 hover:text-ink' }}">
            Student
        </a>
        <a href="{{ route('login.admin') }}"
           class="px-4 py-2 rounded-full text-sm font-semibold transition-colors {{ $activePortal === 'admin' ? 'bg-white text-ink shadow-sm' : 'text-ink/50 hover:text-ink' }}">
            Admin / Staff
        </a>
    </div>

    <h1 class="font-display text-3xl font-semibold text-ink tracking-tight mb-2">
        {{ $activePortal === 'admin' ? 'Staff sign in' : 'Welcome back' }}
    </h1>
    <p class="text-ink/60 mb-8">
        {{ $activePortal === 'admin' ? 'Sign in to manage the site.' : 'Sign in to continue your Sinel Academy training.' }}
    </p>
    ...
    @if ($activePortal === 'student')
        <p class="mt-8 text-sm text-ink/50">
            ...
            <a href="{{ route('register') }}" class="text-emerald font-semibold">Create an account</a>
        </p>
    @endif
