...
<div class="bg-card border border-line rounded-3xl overflow-hidden">
    @if ($projects->isEmpty())
        <div class="p-10 text-center text-ink/50 text-sm">
            No projects yet. <a href="{{ route('admin.projects.create') }}" class="text-emerald font-medium">Add your first one</a>.
        </div>
    @else
            ...
            <tbody>
                @foreach ($projects as $project)
                    <tr class="border-b border-line last:border-0">
                        <td class="px-6 py-4 text-ink/40 font-mono text-xs">{{ $project->sort_order }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                @if ($project->cover_image_url)
                                    <img src="{{ $project->cover_image_url }}" alt="" class="w-9 h-9 rounded-lg object-cover shrink-0">
                                @else
                        ...
                        <td class="px-6 py-4 text-ink/60 hidden sm:table-cell">{{ $project->location }}</td>
                        <td class="px-6 py-4 text-ink/60 hidden md:table-cell">{{ $project->category }}</td>
                        <td class="px-6 py-4">
